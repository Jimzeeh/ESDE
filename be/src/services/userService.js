...
// Added code
module.exports.GetAdminIds = (userId) => {
    
    return new Promise((resolve, reject) => {
        console.log("GetAdminIds running.");
        //I referred to https://www.codota.com/code/javascript/functions/mysql/Pool/getConnection
        //to prepare the following code pattern which does not use callback technique (uses Promise technique)
        pool.getConnection((err, connection) => {
            if (err) {
                ...
                connection.query(`SELECT user_id from user where role_id = 1 and user_id = ?;`, [userId],(err, rows) => {
                    if (err) {
                        reject(err);
                    } else {
                        resolve(rows);
...
module.exports.MatchFileIdUserId = (userId, fileId) => {
    
    return new Promise((resolve, reject) => {
        console.log("MatchFileIdUserId running.");
        //I referred to https://www.codota.com/code/javascript/functions/mysql/Pool/getConnection
        //to prepare the following code pattern which does not use callback technique (uses Promise technique)
        pool.getConnection((err, connection) => {
            if (err) {
                ...
                connection.query(`SELECT file_id from file where created_by_id = ?;`, [fileId, userId],(err, rows) => {
                    if (err) {
                        reject(err);
                    } else {
                        resolve(rows);
